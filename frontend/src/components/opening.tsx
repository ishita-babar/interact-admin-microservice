...
const OpeningComponent = ({ opening, setOpenings }: Props) => {
  const removeFlag = () => {
    const toaster = Toaster.startLoad('Removing Flag', opening.id);
    const URL = `/flags/openings/${opening.id}`;
    postHandler(URL, {})
      .then(res => {
        if (res.statusCode === 200) {
          setOpenings(prev => prev.filter(p => p.id != opening.id));
    ...
    <div className="w-full">
      <div className="w-full relative bg-white font-primary flex gap-1 rounded-lg rounded-b-none p-2 text-primary_black border-gray-300 border-b-0 border-[1px]">
        <div className="h-full">
          <Link href={`${`/explore/user/${opening.user.username}`}`} className="rounded-full">
            <Image
              crossOrigin="anonymous"
              width={100}
              height={100}
              alt={'User Pic'}
              src={`${USER_PROFILE_PIC_URL}/${opening.user.profilePic}`}
        ...
        <div className="w-[calc(100%-32px)] flex flex-col gap-1">
          <div className="w-full h-fit flex justify-between">
            <Link href={`${`/explore/user/${opening.user.username}`}`} className="font-medium flex items-center gap-1">
              {opening.user.name}
